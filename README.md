...
Production builds are emitted through Create React App's `react-scripts build`, which already minifies the JavaScript and CSS bundles. This repo also disables production source maps via `.env.production`, so anything published from the `build/` directory is shipped in an uglified/minified form whether it goes to GitHub Pages or another static host.
