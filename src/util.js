...
export const triggerRoomUnlock = (roomWhereKeyIsFound) => {
  switch (roomWhereKeyIsFound) {
    case 'BEDROOMCLOSET':
      alert(
        'You found the key that unlocks the door in the living room! Go check it out!',
      );
      playKeyClickSound();
      setImageForRoom(
        maps.LIVINGROOM,
        maps.IMAGECHANGES.livingRoomUnlockedImage,
      );
      maps.LIVINGROOM.up = {
    ...
    case 'KITCHENCORNER':
      alert(
        'You found the key that unlocks the door in the bedroom! Go check it out!',
      );
      playKeyClickSound();
      setImageForRoom(
        maps.BEDROOMCORNER,
        maps.IMAGECHANGES.bedroomCornerUnlockedImage,
      );
...
export const secondHouseTrigger = (
  ...
  if (direction === 'in') {
    ...
    setImageForRoom(
      maps.LADDERTOSECONDHOUSE,
      maps.IMAGECHANGES.stairToSecondHouseLockedImage,
    );
...
export const generateCountdownClock = (setIsCountdownRunning) => {
  ...
  const percentIncrement =
    (100 * timeIncrement) / (totalSecondsForCountdown * 1000);

  clockCountdown(
    barWidth,
    totalSecondsForCountdown,
    barFrameRateInFPS,
    timeIncrement,
    percentIncrement,
  );
...
export const clockCountdown = (
  ...
  const countdown = setInterval(() => {
    ...
    const loadingBarSubtext = document.getElementById(
      'popUpWindowLoadingBarSubtext',
    );
    if (loadingBarSubtext) {
      loadingBarSubtext.innerHTML = 'ERROR: LOGOUT ATTEMPT HALTED';
    }
    const popupCounter = document.getElementById('popUpWindowCounter');

    if (!progressBar && !loadingBarSubtext && !popupCounter) {
    ...
    if (barWidth >= 100) {
      clearInterval(countdown);
      if (loadingBarSubtext) {
        loadingBarSubtext.innerHTML =
          'Connection Successfully Terminated. Goodbye!';
      }
      setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (popUpWindowHeader) {
          popUpWindowHeader.innerHTML = 'WATCH OUT';
        }
      }, 60000);
      setTimeout(() => {
        const popUpWindowParagraph = document.getElementById(
          'popUpWindowParagraph',
        );
        if (popUpWindowParagraph) {
          popUpWindowParagraph.innerHTML = `You don't know who you can trust! Not even Mrs. Tobias. Please stand by as we attempt to terminate your connection…<br>
      ...
      setTimeout(() => {
        ...
        if (popUpWindowParagraph) {
          popUpWindowParagraph.innerHTML = `You don't know who you can trust! Not even Mrs. Tobias.<br>
      ...
      setTimeout(() => {
        ...
        if (loadingBarSubtext) {
          loadingBarSubtext.innerHTML = 'IT WAS AN ACCIDENT';
        }
      }, 300000);
    } else {
      ...
      const secondsLeftInCountdown = (
        (timeIncrement * (barPercentLeft / percentIncrement)) /
        1000
      ).toFixed(0);
      if (popupCounter) {
        popupCounter.innerHTML = secondsLeftInCountdown;
      }
    }
  }, timeIncrement);
};
    ...
    screen.push(
      <div
        ...
        style={{
          ...
          scale(${Math.floor(Math.random() * 10)}, ${Math.floor(
            Math.random() * 10,
          )}`,
        }}
      >
