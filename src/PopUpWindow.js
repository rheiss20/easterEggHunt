...
export class PopUpWindow extends React.Component {
  ...
  startDragging = ({ clientX, clientY }) => {
    this.handleRef.style.transform = `translate(${this.dragStartLeft +
      clientX -
      this.dragStartX -
      this.initialLeft}px, ${this.dragStartTop +
      clientY -
      this.dragStartY -
      this.initialTop}px)`;
  };
  ...
  render() {
    const left =
      this.props.windowWidth * 0.5 - this.state.dimensions.width * 0.5;
    const top =
      this.props.windowHeight * 0.5 - this.state.dimensions.height * 0.5;
    ...
    return (
      ...
      >
        ...
        >
          Unauthorized access of forbidden files has been detected. Please stand
          by as we attempt to terminate your connection…
          <br />
          <br />
          Do not attempt to close this prompt by clicking an X. <br />
          <br />
        ...
        >
          You have{' '}
          <span id='popUpWindowCounter' style={{ margin: '0px 5px' }}>
            60
          </span>{' '}
          seconds remaining before you are disconnected
        </p>
      </div>
    );
  }
